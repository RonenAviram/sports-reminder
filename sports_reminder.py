...
def build_email_html(matches: list[dict], today: str, player_stats: list[dict] | None = None) -> str:
    ...
    rows = ""
    for m in matches:
        emoji = sport_emoji.get(m["sport"], "Ã°ÂÂÂÃ¯Â¸Â")
...
def send_email(to: str, matches: list[dict], today: str, player_stats: list[dict] | None = None):
    ...
    for m in matches:
        plain += f"  {m['away']} @ {m['home']}  Ã¢ÂÂ  {m['league_name']}  Ã¢ÂÂ  {m['time']} (IL)\n"
    if player_stats:
        plain += "\n---\n"
...
def main():
    ...
    stats_only     = "--stats-only"  in args   # 07:00 IL Ã¢ÂÂ post-game stats only
    no_stats       = "--no-stats"    in args   # 09:00 IL Ã¢ÂÂ gorning games only
    ...
    if stats_only:
        ...
        if not avdija_enabled:
            print("\nÃ°ÂÂÂ Avdija stats disabled in user settings Ã¢ÂÂ skipping stats email.")
            return
        print("\nÃ°ÂÂÂ Stats-only mode Ã¢ÂÂ fetching last game stats...")
        player_stats = []
        for p in PLAYER_WATCH:
            ps = fetch_player_last_game_stats(p)
            if ps:
                label = "DNP" if ps.get("dnp") else f"{ps['pts']} pts / {ps['reb']} reb / {ps['ast']} ast"
                print(f"   Ã°ÂÂÂ {p['display_name']}: {label} ({ps['game_date_il']})")
                player_stats.append(ps)
            else:
                print(f"   Ã¢ÂÂ Ã¯Â¸Â  {p['display_name']}: no recent game found")
        if send_mode:
            if player_stats:
                print(f"\nÃ°ÂÂÂ§ Sending stats email to {GMAIL_SENDER}...")
                send_email(GMAIL_SENDER, [], today, player_stats)
            else:
                print("\nÃ°ÂÂÂ­ No player stats found Ã¢ÂÂ no email sent.")
        else:
            print("Ã¢ÂÂ¹Ã¯Â¸Â  Dry-run (stats-only). Add --send to send.")
        return
    ...
    for p in watch_list:
        ...
        if ps:
            ...
            print(f"   Ã¢ÂÂ Ã¯Â¸Â  {p['display_name']}: ÃÂÃÂ ÃÂ ÃÂÃÂ¦ÃÂ ÃÂÃÂ©ÃÂ¹ÃÂÃÂ§ ÃÂÃÂÃÂ¨ÃÂÃÂ")
